...
| 49 | **Early_Delinquency_Flag** | 数值 | 早期违约特征（前6个月未全额还款次数） | 统计前6个有效月份中，实际还款少于预期还款的次数 | ⭐⭐⭐⭐ |
| 50 | **Amortization_Lag** | 数值 | 摊销滞后（从贷款发放到首次还本的期数） | np.argmax(Observed > 0) | ⭐⭐⭐ |
| 51 | **Early_Payment_Trend** | 数值 | 早期支付趋势（前6个月的支付趋势） | 前6个月实际支付的线性趋势（斜率） | ⭐⭐⭐ |
...
2. **Early_Delinquency_Flag**（早期违约特征，索引 49）
...
4. **Early_Payment_Trend**（早期支付趋势，索引 51）
...
| 索引 | 特征名称 | 类型 | 说明 | 计算公式 | 重要性 |
|------|---------|------|------|----------|--------|
| 52 | **MaturityPressure_Index** | 数值 | 到期压力指数 | CurrentActualUPB_last / (RemainingMonthsToLegalMaturity_last + 1) | ⭐⭐⭐ |
...
### 6.1 CurrentNonInterestBearingUPB 时序特征（索引 53）
...
### 6.2 CurrentActualUPB 时序特征（索引 54-65）
...
|------|---------|------|--------|------|-------|-------|--------|
| 54 | CurrentActualUPB_w_main_trend | w_main (0,3,6,9,12) | trend | 趋势：(last-first)/(\|first\|+1) | 0.1395 | 0.5524 | ⭐⭐⭐ |
| 55 | CurrentActualUPB_w_main_vol | w_main | vol | 波动性：std/(\|mean\|+1) | 0.1347 | 0.5439 | ⭐⭐⭐ |
| 56 | CurrentActualUPB_w_main_dmean | w_main | dmean | 一阶差分均值 | 0.1411 | 0.5570 | ⭐⭐⭐ |
| 57 | CurrentActualUPB_w_main_dstd | w_main | dstd | 一阶差分标准差 | 0.1218 | 0.4918 | ⭐ |
| 58 | CurrentActualUPB_w_alt1_trend | w_alt1 (0,2,4,6,8,10,12) | trend | 趋势 | 0.1395 | 0.5524 | ⭐⭐⭐ |
| 59 | CurrentActualUPB_w_alt1_vol | w_alt1 | vol | 波动性 | 0.1366 | 0.5491 | ⭐⭐⭐ |
| 60 | CurrentActualUPB_w_alt1_dmean | w_alt1 | dmean | 一阶差分均值 | 0.1408 | 0.5565 | ⭐⭐⭐ |
| 61 | CurrentActualUPB_w_alt1_dstd | w_alt1 | dstd | 一阶差分标准差 | 0.1201 | 0.4910 | ⭐ |
| 62 | CurrentActualUPB_w_alt2_trend | w_alt2 (0,3,6,9) | trend | 趋势 | 0.1330 | 0.5374 | ⭐⭐ |
| 63 | CurrentActualUPB_w_alt2_vol | w_alt2 | vol | 波动性 | 0.1284 | 0.5217 | ⭐ |
| 64 | CurrentActualUPB_w_alt2_dmean | w_alt2 | dmean | 一阶差分均值 | 0.1311 | 0.5301 | ⭐⭐ |
| 65 | CurrentActualUPB_w_alt2_dstd | w_alt2 | dstd | 一阶差分标准差 | 0.1215 | 0.4918 | ⭐ |
...
| 8 | **CurrentActualUPB_w_main_dmean** | 56 | 0.1411 | 0.5570 | 时序特征 |
| 9 | **CurrentActualUPB_w_alt1_dmean** | 60 | 0.1408 | 0.5565 | 时序特征 |
| 10 | **NumberOfBorrowers** | 21 | 0.1403 | 0.5559 | 静态特征 |
| 11 | **CurrentActualUPB_w_alt1_trend** | 58 | 0.1395 | 0.5524 | 时序特征 |
| 12 | **CurrentActualUPB_w_main_trend** | 54 | 0.1395 | 0.5524 | 时序特征 |
...
**注意**：
- 新增的特征（High_LTV_Flag, Single_Borrower_Flag, MaturityPressure_Index, Zero_Payment_Streak, Early_Delinquency_Flag, Amortization_Lag, Early_Payment_Trend）尚未进行单特征分析，其性能表现待评估
- 特征索引已更新以反映最新的特征列表（新增了3个摊销行为延伸特征）
...
- **摊销特征**：9个（5个基础特征：索引 43-47 + 4个延伸特征：索引 48-51）
...
- **到期风险特征**：1个（索引 52，MaturityPressure_Index）
- **时序特征**：14个（CurrentNonInterestBearingUPB: 1个，索引 53 + CurrentActualUPB: 12个，索引 54-65）
...
- MaturityPressure_Index（索引 52）：到期风险特征
- Zero_Payment_Streak（索引 48）：摊销行为延伸特征（零支付连续月数）
- Early_Delinquency_Flag（索引 49）：早期违约特征（前6个月未全额还款次数）- **高优先级**
- Amortization_Lag（索引 50）：摊销滞后特征（从贷款发放到首次还本的期数）- **中优先级**
- Early_Payment_Trend（索引 51）：早期支付趋势特征（前6个月的支付趋势斜率）- **中优先级**
...
2. **强烈推荐**：新增特征（High_LTV_Flag, Single_Borrower_Flag, MaturityPressure_Index, Zero_Payment_Streak, Early_Delinquency_Flag）
3. **推荐**：增强特征（Amortization_Lag, Early_Payment_Trend）- 用于增强模型稳健性
   - 这些特征捕捉了更细粒度的风险信号，理论上应该对异常检测有帮助
   - 建议运行单特征分析评估其实际性能
4. **强烈推荐**：CreditScore, DTI_HighRisk_Flag, CurrentActualUPB时序特征, OriginalInterestRate
5. **推荐**：NumberOfBorrowers, BorrowerCount_Ajusted_DTI, LTV_x_DTI, Affordability_Index
6. **可考虑删除**：AUPRC < 0.13 的特征（表现接近随机）
...
- `test_ids.npy`：测试集ID
- `feature_names.txt`：特征名称列表（67个特征，删除后约53个）
...
- **48-51**：摊销行为延伸特征（Zero_Payment_Streak, Early_Delinquency_Flag, Amortization_Lag, Early_Payment_Trend）
- **52**：到期风险特征（MaturityPressure_Index）
- **53**：CurrentNonInterestBearingUPB时序特征
- **54-65**：CurrentActualUPB时序特征
