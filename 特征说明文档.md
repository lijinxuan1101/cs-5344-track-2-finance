...
| 38 | **Borrowers_Risk_Flag** | 标志 | 借款人风险标志 | NumberOfBorrowers == 1 | - | - | ⭐ |
...
**说明**：
- `DTI_HighRisk_Flag` 是目前表现最好的偿债压力特征（AUPRC=0.1474）
- `Borrowers_Risk_Flag` 与静态特征中的 `Single_Borrower_Flag` 协同使用，可交叉验证单借款人风险
...
| 索引 | 特征名称 | 类型 | 说明 | 计算公式 | AUPRC | AUROC | 重要性 |
|------|---------|------|------|----------|-------|-------|--------|
| 41 | **LTV_Change** | 数值 | LTV变化量 | EstimatedLTV_last - OriginalLTV | 0.1308 | 0.5086 | ⭐ |
| 42 | **High_LTV_Flag** | 标志 | 高杠杆阈值标志 | OriginalLTV ≥ 95 | - | - | ⭐⭐⭐ |
...
|------|---------|------|------|----------|--------|
| 48 | **Zero_Payment_Streak** | 数值 | 零支付连续月数（断供前兆） | 从后往前找最近的连续零支付月数 | 0.2693 | 0.6029 | ⭐⭐⭐ |
| 49 | **Early_Delinquency_Flag** | 数值 | 早期违约特征（前6个月未全额还款次数） | 统计前6个有效月份中，实际还款少于预期还款的次数 | 0.4999 | 0.8021 | ⭐⭐⭐⭐ |
...
| 索引 | 特征名称 | 类型 | 说明 | 计算公式 | 重要性 |
|------|---------|------|------|----------|--------|
| 54 | **MaturityPressure_Index** | 数值 | 到期压力指数 | CurrentActualUPB_last / (RemainingMonthsToLegalMaturity_last + 1) | ⭐⭐⭐ |
...
| 索引 | 特征名称 | 类型 | 说明 | 计算公式 | 重要性 |
|------|---------|------|------|----------|--------|
| 52 | **UPB_trend_x_amort_short_mean** | 数值 | 资产波动复合特征（高余额下降慢的贷款） | CurrentActualUPB_w_main_trend × amort_short_mean | ⭐⭐⭐⭐ |

**特征详细说明**：

**UPB_trend_x_amort_short_mean**（资产波动复合特征，索引 52）
...
| 6 | **DTI_HighRisk_Flag** | 36 | 0.1474 | 0.5712 | 偿债压力 |
...
| 13 | **BorrowerCount_Ajusted_DTI** | 39 | 0.1392 | 0.5312 | 偿债压力 |
| 14 | **Affordability_Index** | 40 | 0.1368 | 0.5238 | 偿债压力 |
...
|--------|---------|------|-------|-------|------|
| **检测器 1 (王牌)** | Early_Delinquency_Flag | 49 | **0.4999** | **0.8021** | 早期违约检测器，表现最佳 |
| **检测器 2 (二号王牌)** | amort_short_mean | 43 | **0.4749** | **0.7525** | 摊销短缺检测器，表现优异 |
| **检测器 3 (三号王牌)** | Zero_Payment_Streak | 48 | 0.2693 | 0.6029 | 零支付连续月数检测器 |
...
- **偿债压力特征**：5个（索引 36-40：DTI_HighRisk_Flag, DTI_MidRisk_Flag, Borrowers_Risk_Flag, BorrowerCount_Ajusted_DTI, Affordability_Index）
- **杠杆风险特征**：2个（索引 41-42，LTV_Change 和 High_LTV_Flag）
...
- **复合特征**：1个（索引 52，UPB_trend_x_amort_short_mean）
...
- High_LTV_Flag（索引 42）：杠杆极端化风险特征（高杠杆阈值标志，OriginalLTV ≥ 95）
...
- UPB_trend_x_amort_short_mean（索引 52）：资产波动复合特征（高余额下降慢的贷款）- **高优先级**
...
- **36-40**：偿债压力特征（DTI_HighRisk_Flag, DTI_MidRisk_Flag, Borrowers_Risk_Flag, BorrowerCount_Ajusted_DTI, Affordability_Index）
- **41-42**：杠杆风险特征（LTV_Change, High_LTV_Flag）
- **43-47**：摊销信号特征（amort_short_mean, amort_short_70, amort_short_50, amort_short_std, io_payment_count）
- **48-51**：摊销行为延伸特征（Zero_Payment_Streak, Early_Delinquency_Flag, Amortization_Lag, Early_Payment_Trend）
- **52**：资产波动复合特征（UPB_trend_x_amort_short_mean）
